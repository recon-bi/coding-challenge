...
function Hotels() {
  ...
  const [search, setSearch] = React.useState({startDate:dayjs().add(1, 'days').format("YYYY-MM-DD"), endDate:dayjs().add(3, 'days').format("YYYY-MM-DD")})
  ...
  return (
    <DashboardLayout>
        ...
        <MDBox mb={1} mt={1}>
          <DataCard title="Search for hotels">
            ...
            <MDBox width={175} ml={1}>
              <CommonAutocomplete options={cityOptions} label="City" onSelected={({ label }: AutocompleteOptionType) => setSearchParams('city', label)}  />
            </MDBox>
            <MDBox width={175} ml={1}>
              <CommonAutocomplete options={countryOptions} label="Country" onSelected={({ label }: AutocompleteOptionType) => setSearchParams('country', label)} />
            </MDBox>
      ...
      <DataTable columns={columns} rows={data} onRowClick={handleRowClick} manualFilters={false} manualPagination={false} />
      <BookingForm selectedItem={selectedItem} showModal={showBookingForm} onClose={() => setShowBookingForm(false)} selections={search} />
    </DashboardLayout>
  );
}
